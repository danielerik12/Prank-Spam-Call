...
class prankcall {
    ...
    private function ekse() {
        ...
        $h[] = "User-Agent: Grab/5.20.0 (Android 5.0.1; Build $rand)";
...
";
    ...
    public function run() {
        while (true) {
            echo "[??] Anjing Mau Spam Banyak ? (n/y) ==> ";
            $loop = $this->get();
            if ($loop == "y" OR $loop == "n") {
                break;
            } else {
                echo "Jika ya jawab n jika tidak jawab y
";
                continue;
            }
        }
        if ($loop == "y") {
            echo "[??] Anjing Mau Spam Berapa ==> ";
            $many = $this->get();
            $this->loop($many);
        } else {
            $this->ekse();
